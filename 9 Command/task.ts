...
class Calculator {
  ...
  subtract(operand: number): number {
    return this.run(new Subtract(this.arithmeticUnit, operand));
  }

  multiply(operand: number): number {
    return this.run(new Multiply(this.arithmeticUnit, operand));
  }

  divide(operand: number): number {
    return this.run(new Divide(this.arithmeticUnit, operand));
  }
